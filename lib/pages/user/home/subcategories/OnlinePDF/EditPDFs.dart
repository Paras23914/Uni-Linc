...
class EditPDFs extends StatefulWidget {
  const EditPDFs({
    super.key,
    required this.title,
    required this.id,
  ...
  @override
  State<EditPDFs> createState() => _EditPDFsState();
}

class _EditPDFsState extends State<EditPDFs> {
  ...
  updatePDF() async {
    final eventdata = firestore.collection('PDFs').doc(widget.id);
    try {
      await eventdata.update(data!);
      Utils().toastmessage("PDF Updated Successfully");
