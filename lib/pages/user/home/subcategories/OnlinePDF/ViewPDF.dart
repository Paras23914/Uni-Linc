...
class ViewPDFDetail extends StatefulWidget {
  const ViewPDFDetail({super.key, required this.id});
  final String id;

  @override
  State<ViewPDFDetail> createState() => _ViewPDFDetailState();
}

class _ViewPDFDetailState extends State<ViewPDFDetail> {
  ...
  Future<void> fetchPDFDetails() async {
    final PDFDoc = firestore.collection('PDFs').doc(widget.id);
    final snapshot = await PDFDoc.get();
    if (snapshot.exists) {
      setState(() {
